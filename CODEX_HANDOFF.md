...
- 12개 regression + 12개 visible synthetic challenge(`HOLDOUT`) 한국어 fixture, fixture JSON Schema, raw content를 포함하지 않는 결정론적 평가 report, revision 기준 날짜 파서, `field-policy-v1` ambiguity gate, Draft 2020-12 runtime contract와 strict domain validation이 구현되어 있다. `providerMetadata`의 다섯 version은 각각 1–64자, 필수 `toolCalls`는 0–100이며 proposal은 64 KiB, metadata는 8 KiB로 제한된다.
- `fake-v3`의 첫 challenge 기준선은 schema-valid 12/12이지만 route accuracy 0.583333, wrong-local 9/12, top-type accuracy 0.25, signal recall 0.25다. regression wrong-local 0만 현재 CI gate이며, 공개된 challenge gold를 Fake 규칙에 맞춰 바꾸거나 이 결과만으로 실제 LLM을 연결하지 않는다. 자세한 진입 조건은 `docs/EVALUATION.md`를 따른다.
...
- 계정별 연속 5회 실패 시 15분 잠금은 구현되어 있다. 공개 배포 전 account hardening의 다음 순서는 local email verification, password reset delivery, IP·edge rate limit/abuse protection, MFA/passkey 검토와 account deletion이다. 제품 분석의 다음 순서는 challenge set으로 드러난 deterministic rule 일반화, owner-scoped review-outcome 집계, owner-scoped tag/alias 후보 조회, field-level Fake cloud 계약과 실패 상태, async 분석 수명주기·관측성이다.
