...
- 계정별 연속 5회 실패 시 15분 잠금은 구현되어 있다. 공개 배포 전 account hardening의 다음 순서는 local email verification, password reset delivery, IP·edge rate limit/abuse protection, MFA/passkey 검토와 account deletion이다. 제품 분석의 다음 순서는 version-2 date mention/item/item-source-span gold의 독립 adjudication, binding label policy를 포함한 독립 adjudicated evaluation dataset v3, 사전 threshold를 등록한 별도 blind release, owner-scoped tag/alias 후보 조회, field-level Fake cloud 계약과 실패 상태, async 분석 수명주기·관측성이다. `review-default-v3`가 schema v2의 명시적 due binding과 과거 v1의 보수적 fallback을 구분해도, owner-scoped outcome 집계와 blind harness 경계만으로 실제 blind 결과·실사용 교정 표본·provider privacy/consent/cost/failure 조건이 충족되지는 않는다. 실제 LLM gate는 계속 닫혀 있다.
