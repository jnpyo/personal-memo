...
- 현재 실제 external provider와 consent grant/revoke API는 없다. grant timestamp/authorization decision의 run snapshot·descriptor binding, DB transaction 밖 bounded async/timeout 호출, idempotent provider-request token, queued/running·retry·duration·token·cost와 top-k context도 미구현이며 실제 provider 전 hard gate다.
