...
- Flyway `V1`–`V11`이 memo/revision, proposal/application, canonical item/tag/task, owner integrity, revision capture context, analyzer·prompt·local model·embedding model·routing policy provenance, local/Google identity, JDBC session schema, claimed user identity 무결성과 일회성 initial-account provisioning gate를 관리한다. `V11`은 owner별 proposal의 최신 application을 찾는 review-outcome 조회 인덱스만 추가하며 raw analytics 복제본이나 clickstream table을 만들지 않는다.
...
- 12개 regression + 12개 visible synthetic challenge(`HOLDOUT`) 한국어 fixture, fixture JSON Schema, raw content를 포함하지 않는 결정론적 평가 report, revision 기준 날짜 파서, `field-policy-v1` ambiguity gate, Draft 2020-12 runtime contract와 strict domain validation이 구현되어 있다. `fake-v4` / `korean-rules-v2`는 모든 다음 주 요일·한국어 시각, 근사 주말·event-relative deadline, 일반 action/reference/event/multi-intent 규칙을 다룬다. `providerMetadata`의 다섯 version은 각각 1–64자, 필수 `toolCalls`는 0–100이며 proposal은 64 KiB, metadata는 8 KiB로 제한된다.
- `fake-v3`의 첫 challenge 기준선은 wrong-local 9/12였고, 현재 `fake-v4`는 regression과 visible challenge 모두 schema-valid 12/12, wrong-local 0, route exact 12/12, preferred top-1 type correct 12/12, signal-set exact 12/12다. challenge 문장 전체/3-token source 복사 방지와 다른 문구의 양·음성 단위 테스트를 함께 사용한다. 그래도 공개된 합성 challenge는 blind/general accuracy가 아니며 report-only다. regression wrong-local 0만 현재 CI gate이고, exact date/item gold·별도 blind set·실사용 교정 표본 없이 실제 LLM을 연결하지 않는다. 자세한 진입 조건은 `docs/EVALUATION.md`를 따른다.
...
- 계정별 연속 5회 실패 시 15분 잠금은 구현되어 있다. 공개 배포 전 account hardening의 다음 순서는 local email verification, password reset delivery, IP·edge rate limit/abuse protection, MFA/passkey 검토와 account deletion이다. 제품 분석의 다음 순서는 complete date/item evaluation gold, 별도로 보관하는 blind set, owner-scoped tag/alias 후보 조회, field-level Fake cloud 계약과 실패 상태, async 분석 수명주기·관측성이다. Owner-scoped outcome 집계가 생겼어도 실사용 교정 표본·provider privacy/consent/cost/failure 조건을 충족하지 않았으므로 실제 LLM gate는 계속 닫혀 있다.
