...
- 현재 실제 external provider와 consent grant/revoke API는 없다. V14의 snapshot/token은 내부 gateway request와 최종 run row에만 존재하고 HTTP/proposal/metadata/log에 노출되지 않는다. gateway 호출 전 durable preparation commit, immutable descriptor/executor binding, DB transaction 밖 bounded async/timeout 호출, queued/running·retry·duration·model-token·cost와 top-k context는 미구현이며 실제 provider 전 hard gate다.
