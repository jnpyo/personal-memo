...
interface OllamaTruncationDiagnosticApi {
  OllamaModelPreflight preflight();

  OllamaWarmupResult warmup();

  OllamaTruncationDiagnosticResult diagnose(ObjectNode skillEvidence, ObjectNode selectionSchema);
...
/** Test-only v7-A client that changes only the v6 selection token cap. */
final class OllamaLocalShadowTruncationDiagnosticClient implements OllamaTruncationDiagnosticApi {
  static final URI CHAT_URI = OllamaLocalShadowSkillClient.CHAT_URI;
  static final String PROMPT_VERSION = OllamaLocalShadowSkillClient.PROMPT_VERSION;
  static final int SEED = OllamaLocalShadowSkillClient.SEED;
  static final int NUM_PREDICT = 128;
  static final int NUM_CONTEXT = OllamaLocalShadowSkillClient.NUM_CONTEXT;
  static final int MAX_MODEL_OUTPUT_BYTES = OllamaLocalShadowSkillClient.MAX_MODEL_OUTPUT_BYTES;
  static final String SYSTEM_PROMPT = OllamaLocalShadowSkillClient.SYSTEM_PROMPT;
  ...
  OllamaLocalShadowTruncationDiagnosticClient(
  ...
  public OllamaTruncationDiagnosticResult diagnose(
    ...
    ObjectNode selection = parseSelection(content.asText());
    if (selection == null) {
      return OllamaTruncationDiagnosticResult.rejected(
          termination,
    ...
    return OllamaTruncationDiagnosticResult.completedStop(
        selection, elapsed, apiMetrics, contentBytes);
  }
  ...
  private ObjectNode parseSelection(String content) {
    try {
...
enum DiagnosticTermination {
  STOP,
  LENGTH,
  OTHER
}

enum DiagnosticModelRejection {
  NONE,
  TERMINATION_LENGTH,
  TERMINATION_OTHER,
...
record OllamaTruncationDiagnosticResult(
    DiagnosticTermination termination,
    DiagnosticModelRejection rejection,
    ObjectNode selectionOutput,
    long wallDurationNanos,
    OllamaApiMetrics metrics,
    int modelOutputBytes) {
  OllamaTruncationDiagnosticResult {
    Objects.requireNonNull(termination, "termination");
    Objects.requireNonNull(rejection, "rejection");
    Objects.requireNonNull(metrics, "metrics");
    if (wallDurationNanos < 0 || modelOutputBytes < 0 || modelOutputBytes > 64 * 1024) {
      throw new IllegalArgumentException("Diagnostic response metrics are outside the wire cap.");
    }
    if (rejection == DiagnosticModelRejection.NONE) {
      if (termination != DiagnosticTermination.STOP || selectionOutput == null) {
        throw new IllegalArgumentException("Only a completed stop may carry a selection.");
      }
    } else if (selectionOutput != null) {
      throw new IllegalArgumentException("Rejected diagnostic output must not be retained.");
    }
    selectionOutput = selectionOutput == null ? null : selectionOutput.deepCopy();
  }

  static OllamaTruncationDiagnosticResult completedStop(
      ObjectNode selectionOutput,
      long wallDurationNanos,
      OllamaApiMetrics metrics,
      int modelOutputBytes) {
    return new OllamaTruncationDiagnosticResult(
        DiagnosticTermination.STOP,
        DiagnosticModelRejection.NONE,
        selectionOutput,
        wallDurationNanos,
        metrics,
        modelOutputBytes);
  ...
  static OllamaTruncationDiagnosticResult rejected(
      DiagnosticTermination termination,
      DiagnosticModelRejection rejection,
      long wallDurationNanos,
      OllamaApiMetrics metrics,
      int modelOutputBytes) {
    return new OllamaTruncationDiagnosticResult(
        termination, rejection, null, wallDurationNanos, metrics, modelOutputBytes);
  }

  boolean completedStop() {
    return termination == DiagnosticTermination.STOP;
  }

  boolean selectionAvailable() {
    return selectionOutput != null;
  }

  boolean atOrBelowModelOutputCap() {
    return modelOutputBytes <= OllamaLocalShadowTruncationDiagnosticClient.MAX_MODEL_OUTPUT_BYTES;
  }

  @Override
  public ObjectNode selectionOutput() {
    return selectionOutput == null ? null : selectionOutput.deepCopy();
  }
}
