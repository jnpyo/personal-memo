...
- V17의 owner-scoped fence별 `gateway-attempt-v1` 내부 ledger; executor 거절과 gateway가 반환한 `UNAVAILABLE`을 구분하고, monotonic local elapsed time과 원격 결과·model token·cost의 `UNKNOWN`/`NOT_APPLICABLE`/`NOT_REPORTED` 상태를 숫자와 분리
...
- V17의 과거 dispatch `attempt_history_version=none`/0-row 보존, fence별 최대-attempt ledger, executor 거절·provider failure·timeout·interrupt·process-loss·늦은 fence 분리와 evidence nullability 통합 검증
