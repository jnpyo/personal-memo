...
- Milestone 2: in progress. Korean date policy, versioned regression/visible-challenge fixtures, a content-free deterministic evaluation report and wrong-local safety gate, runtime schema/domain validation, versioned field-level routing with persisted provenance, provider-independent Fake cloud enrichment, prompt-injection boundaries, and `UNKNOWN` user resolution are implemented. `fake-v4` generalizes action/reference/event and relative-date rules and now covers all 12 visible challenge labels without copied challenge phrases, while the set remains report-only and is not a blind or general Korean accuracy estimate. Owner-scoped outcome aggregation, complete date/item gold, provider failure/consent boundaries, top-k context, and async lifecycle remain before a real provider decision.
