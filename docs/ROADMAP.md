...
- Milestone 2: in progress. Korean date policy, versioned regression/visible-challenge fixtures, a content-free deterministic evaluation report and wrong-local safety gate, runtime schema/domain validation, versioned field-level routing with persisted provenance, provider-independent Fake cloud enrichment, prompt-injection boundaries, and `UNKNOWN` user resolution are implemented. The first visible challenge baseline intentionally exposes that the rule-based Fake is not ready to authorize a real provider or claim general Korean memo accuracy; it is not a blind accuracy estimate.
