...
- Metadata/consent hardening now canonicalizes every new LOCAL, cloud-success, and fallback
  ...
  transaction, and finalizes only after revision/fence rechecks. None of the internal evidence,
  dispatch proposal/binding, or token is exposed through proposal or HTTP contracts.
...
- background worker and automatic restart recovery, per-attempt history, duration, model-token, and
...
approved provider must honor the durable token idempotently. Background scheduling, automatic
restart recovery, attempt history, duration/model-token/cost metrics, top-k context, and the privacy,
