...
- Metadata/consent hardening now canonicalizes every new LOCAL, cloud-success, and fallback
  ...
  V14 also stores a coherent internal final-run authorization/grant snapshot and deterministic
  gateway-request token without exposing them through proposal or HTTP contracts. It does not yet
  durably commit that snapshot before the call or bind descriptor lookup to immutable execution.
...
- durable pre-call preparation, immutable descriptor/executor binding, async queued/running status,
  out-of-transaction timeout/retry execution, duration, model-token, and cost metrics *(not implemented)*
...
The implemented V13 consent/outcome and V14 execution-snapshot slices also do not authorize a
provider. There is no public consent grant/revoke API or external provider configuration. V14 stores
the descriptor-related authorization/grant values and a deterministic token in the final run, but
the gateway is still called before that row commits and inside the same database transaction.
Durable preparation, immutable descriptor/executor binding, bounded execution outside the database
transaction, timeout/recovery, and final revision recheck remain hard gates so a future approved
adapter cannot be called with ambiguous authority or duplicated by an uncertain retry.
