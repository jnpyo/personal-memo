...
- Milestone 2: in progress. Korean date policy, versioned regression/visible-challenge fixtures, a content-free deterministic evaluation report and wrong-local safety gate, runtime schema/domain validation, versioned field-level routing with persisted provenance, provider-independent Fake cloud enrichment, prompt-injection boundaries, `UNKNOWN` user resolution, and raw-content-free owner-scoped review-outcome aggregation are implemented. `fake-v4` generalizes action/reference/event and relative-date rules and now covers all 12 visible challenge labels without copied challenge phrases, while the set remains report-only and is not a blind or general Korean accuracy estimate. The outcome summary separates current run state, latest application/undo state, and versioned latest-selection comparison; `exact` means unchanged application, not AI accuracy. Complete date/item gold, a separately held blind set, provider failure/consent boundaries, top-k context, and async lifecycle remain before a real provider decision.
...
- The immediate evaluation work is complete date/item gold followed by a separately held blind set. Review-outcome counts are supporting owner-local evidence, not permission to connect a real LLM.
...
target, and current `POSTPONED` state is not an append-only event history. Complete date/item gold and
a separately held blind set are the next evaluation slice before any provider comparison.
