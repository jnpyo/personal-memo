...
- Milestone 2: in progress. Korean date policy, versioned regression/`VISIBLE_CHALLENGE` fixtures, a content-free deterministic evaluation report and narrow regression safety gate, runtime schema/domain validation, versioned field-level routing with persisted provenance, provider-independent Fake cloud enrichment, prompt-injection boundaries, `UNKNOWN` user resolution, and raw-content-free owner-scoped review-outcome aggregation are implemented. `fake-v6` / `korean-rules-v4` preserves the sequential item/source-span and overflow boundaries while emitting proposal schema v2 with proposal-local date candidate IDs and nullable TASK due-date references. Historical schema v1 proposals remain recoverable with the former conservative single-task/single-precise-date projection, while `review-default-v3` uses only explicit v2 bindings. That proposal-contract evolution required no migration or historical JSON rewrite; V13 separately adds exact owner/policy/timestamp external-transfer consent and server-owned cloud transfer/gateway/provider/model/policy/outcome evidence, V14 adds internal final-run authorization/grant/token evidence, and V15 adds durable prepare commit, immutable executor binding/descriptor comparison, claim/lease/fence/deadline, DB-transaction-external bounded execution, caller-driven same-key recovery, and revision-rechecking finalize. V16 resolves at most 10 tag proposals and 20 distinct normalized terms against only the authenticated owner's active tag names/aliases by exact equality, uses the complete result for unique resolution, and deterministically supplies at most K=8 internal hint candidates. It snapshots context raw/hash/version/count before the first gateway call, reuses only that DB snapshot during recovery, and scrubs raw at `FINALIZED` while retaining hash/version/count; existing V15 dispatches remain `none`/`0`/null raw/null hash. The production profile now adds a 25-row, 30-second bounded scanner for `PREPARED` and expired-lease `RUNNING` dispatches, using DB-selected owner/idempotency evidence and the same advisory lock/lifecycle for restart recovery. `NO_NETWORK` Fake needs no consent, while unconsented `EXTERNAL_MEMO_CONTENT` is a zero-call fallback. Typed failure, exception, and invalid cloud output persist a validated local proposal as `HYBRID`/`REVIEW_REQUIRED`, reveal no provider text, and force detailed review without raw/canonical changes. Evaluation dataset v2 still reports date mention/item/item-source-span metrics and the existing narrow regression safety failures, but it contains no date-to-item binding gold: reports declare `SUPPORTED_NOT_SCORED_DATASET_V2`, and binding quality plus the visible challenge remain report-only rather than blind or general Korean accuracy estimates. Strict two-reviewer v2 review and ID-only v3 overlay integrity tooling are prepared, but there are no real human manifests, completed adjudication, version-3 dataset, binding score, or `PASS`; `EVALUATION_LABEL_POLICY.md` is still a draft. The external blind harness enforces an outside-repository, independently human-curated dataset, clean candidate commit, aggregate-only output, and privacy fail-closed behavior, but no real blind dataset or pre-registered metric gate is claimed. The outcome summary separates current run state, latest application/undo state, and versioned latest-selection comparison; `exact` means unchanged application, not AI accuracy. Completed human gold adjudication, an approved/adjudicated binding dataset v3, a separately held blind release and pre-registered thresholds, provider/region/retention/grant UX, related-memo/fuzzy/vector/embedding context, per-attempt history, and duration/model-token/cost metrics remain before a real provider decision.
...
- Metadata/consent hardening now canonicalizes every new LOCAL, cloud-success, and fallback
  ...
  every 30 seconds. None of the internal evidence, recovery key/context, dispatch proposal/binding,
  or token is exposed through public DTOs, proposal JSON or `providerMetadata`, ordinary logs,
  browser storage, or service-worker caches.
- The immediate evaluation work is human execution of the prepared independent version-2 review protocol, followed by approval and two-person adjudication of the prepared version-3 binding-label policy/overlay. Only after those labels are frozen may a human-curated separately held blind release use thresholds approved before its first candidate run. Validator success, test-only manifests/overlays, review-outcome counts, and structurally valid v2 bindings are preparation evidence, not permission to connect a real LLM.
...
- per-attempt history, duration, model-token, and cost metrics *(not implemented)*
...
The implemented V13 consent/outcome, V14 execution snapshot, and V15/V16 durable dispatch slices do not
...
exactly-once guarantee. Attempt history, duration/model-token/cost metrics, related-memo/fuzzy/vector/
embedding context, and the privacy, evaluation, provider/region/retention/budget decisions remain
separate gates.
