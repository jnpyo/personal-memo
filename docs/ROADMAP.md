...
- Metadata/consent hardening now canonicalizes every new LOCAL, cloud-success, and fallback
  ...
  transaction, and finalizes only after revision/fence rechecks. The production scheduler reuses
  that lifecycle for a maximum of 25 eligible rows every 30 seconds. None of the internal evidence,
  recovery key/context, dispatch proposal/binding, or token is exposed through proposal or HTTP
  contracts.
...
- top-k retrieval context *(not implemented)*
...
The implemented V13 consent/outcome, V14 execution snapshot, and V15 durable dispatch slices do not
...
finalize. The transport boundary remains at-least-once after an uncertain crash, so any future
approved provider must honor the durable token idempotently. Production background scheduling and
restart recovery now reuse that same lifecycle with a 25-row/30-second bound; they do not provide an
exactly-once guarantee. Attempt history, duration/model-token/cost metrics, top-k context, and the
privacy, evaluation, provider/region/retention/budget decisions remain separate gates.
