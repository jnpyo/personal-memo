...
- Metadata/consent hardening now canonicalizes every new LOCAL, cloud-success, and fallback
  ...
  It still does not snapshot the authorization/grant pair on the run or bind it to the descriptor.
...
- async queued/running status, out-of-transaction timeout/retry execution, a server-issued
  idempotent provider-request token, duration, token, and cost metrics *(not implemented)*
...
The implemented V13 consent/outcome slice also does not authorize a provider. It has no public
consent grant/revoke API and no external provider configuration. It rejects future-dated grants,
but does not yet persist a descriptor-bound authorization/grant snapshot on the run. That snapshot,
bounded provider execution outside the database transaction, and provider-call idempotency remain
hard gates so a future approved adapter cannot be called with ambiguous authority or duplicated by
an uncertain retry.
